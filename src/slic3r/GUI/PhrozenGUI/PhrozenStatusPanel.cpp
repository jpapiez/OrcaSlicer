...
{
    // Format elapsed time using get_time_dhms
    std::string elapsed_time;
    wxString    elapsed_time_text = PHROZEN_NA_STR;

    try {
        elapsed_time = get_time_dhms(static_cast<float>(elapsed_seconds));
    }
    catch (...) {
        ;
    }
...
wxBoxSizer* PhrozenStatusBasePanel::create_temp_axis_group(wxWindow* parent)
{
    ...
    sizerTopLeft->Add(new wxStaticText(box, wxID_ANY, _L("Temperature")), 0, wxLEFT | wxEXPAND, 5);
    ...
    sizerTopLeft->Add(new wxStaticText(box, wxID_ANY, _L("Speed")), 0, wxLEFT | wxEXPAND, 5);
    ...
    sizerTopRight->Add(new wxStaticText(box, wxID_ANY, _L("Manual Adjustment")), 0, wxLEFT, 5);
    ...
    wxStaticText* coolingTitle = new wxStaticText(box, wxID_ANY, _L("Cooling"));
...
wxFlexGridSizer* PhrozenStatusBasePanel::GenNozzleTempControllor(wxWindow* pParent)
{
    ...
    sizerFlex->Add( new wxStaticText(pParent, wxID_ANY, _L("Nozzle")), 1, wxLEFT | wxALIGN_CENTER_VERTICAL, 10 );
...
wxFlexGridSizer* PhrozenStatusBasePanel::GenHeatedBedTempControllor(wxWindow* pParent)
{
    ...
    sizerFlex->Add( new wxStaticText(pParent, wxID_ANY, _L("Heated Bed")), 1, wxLEFT | wxALIGN_CENTER_VERTICAL, 10 );
...
wxBoxSizer* PhrozenStatusBasePanel::GenSpeed_PrintLevel(wxWindow* pParent)
{
    ...
    title->Add(new wxStaticText(pParent, wxID_ANY, _L("Print")), 1, wxLEFT | wxALIGN_CENTER_VERTICAL, 10);
...
wxFlexGridSizer* PhrozenStatusBasePanel::GenCooling_Auxiliary(wxWindow* pParent)
{
    ...
    sizerFlex->Add( new wxStaticText(pParent, wxID_ANY, _L("Auxiliary")), 1, wxLEFT | wxALIGN_CENTER_VERTICAL, 10 );
...
wxFlexGridSizer* PhrozenStatusBasePanel::GenCooling_Part(wxWindow* pParent)
{
    ...
    sizerFlex->Add( new wxStaticBitmap(pParent, wxID_ANY, m_Fan.bmp() ), 0, wxALL | wxALIGN_CENTER_VERTICAL, 10 );
    sizerFlex->Add( new wxStaticText(pParent, wxID_ANY, _L("Part")), 1, wxLEFT | wxALIGN_CENTER_VERTICAL, 10 );
...
wxFlexGridSizer* PhrozenStatusBasePanel::GenCooling_Shield(wxWindow* pParent)
{
    ...
    sizerFlex->Add( new wxStaticBitmap(pParent, wxID_ANY, m_Fan.bmp() ), 0, wxALL | wxALIGN_CENTER_VERTICAL, 10 );
    sizerFlex->Add( new wxStaticText(pParent, wxID_ANY, _L("Shield")), 1, wxLEFT | wxALIGN_CENTER_VERTICAL, 10 );
    ...
    if (abort_dlg == nullptr) {
        ...
        abort_dlg->Bind(EVT_SECONDARY_CHECK_CONFIRM, [this](wxCommandEvent &e) {
            ...
            BOOST_LOG_TRIVIAL(info) << "PhrozenStatusPanel::on_subtask_abort: "
                                    << "User confirmed abort, executing stop command, dev_id=" << obj->get_dev_id();
...
void PhrozenStatusPanel::update_temp_ctrl(MachineObject *obj)
{
    ...
    // bed
    auto* pobj = as_phrozen(obj);
    int nTempBedCurrent = pobj ? (int)pobj->GetPhrozenBedTemperature() : 0;
    int nTempBedTarget = pobj ? (int)pobj->GetPhrozenBedTargetTemperature() : 0;
    ...
    // nozzle
    int nTempNozzleCurrent = pobj ? (int)pobj->GetPhrozenNozzleTemperature() : 0;
    int nTempNozzleTarget = pobj ? (int)pobj->GetPhrozenNozzleTargetTemperature() : 0;
...
void PhrozenStatusPanel::update_print_speed_ctrl(MachineObject *obj)
{
    ...
    auto* pobj = as_phrozen(obj);
    PhrozenPrintSpeed eLevel = print_speed_percent_to_enum( pobj ? pobj->GetPhrozenPrintSpeed() : 0 );
...
void PhrozenStatusPanel::update_fan_cooling_speed_ctrl(MachineObject *obj)
{
    ...
    auto* pobj = as_phrozen(obj);
    int current_auxiliary_cooling = (int)( (pobj ? pobj->GetPhrozenAuxiliaryCoolingSpeed() : 0.0f) * 100 );
    int current_part_cooling = (int)( (pobj ? pobj->GetPhrozenPartCoolingSpeed() : 0.0f) * 100 );
    int current_shield_cooling = (int)( (pobj ? pobj->GetPhrozenShieldCoolingSpeed() : 0.0f) * 100 );
...
void PhrozenStatusPanel::update_z_offset_ctrl(MachineObject *obj)
{
    ...
    auto* pobj = as_phrozen(obj);
    float fZOffset = pobj ? pobj->GetPhrozenZOffset() : 0.0f;
...
void PhrozenStatusPanel::update_webcam_lighting_status( MachineObject *obj )
{
    ...
    if (m_lighting_state_timeout > 0) {
        ...
        auto* pobj = as_phrozen(obj);
        bool bIsLighingEnabled = pobj ? pobj->GetPhrozenCommand_lighting_enabled() : false;
...
void PhrozenStatusPanel::update_print_status(MachineObject *obj)
{
    ...
    // ============================================
    auto* pobj = as_phrozen(obj);
    std::string print_status = pobj ? pobj->GetPhrozenPrintStatus() : std::string("");
...
void PhrozenStatusPanel::update_print_progress(MachineObject *obj)
{
    ...
    auto* pobj = as_phrozen(obj);
    float print_progress = (pobj && pobj->GetPhrozenPrintStatus() == "complete") 
                          ? 1.0f 
                          : (pobj ? pobj->GetPhrozenPrintProgress() : 0.0f);
...
void PhrozenStatusPanel::update_print_file(MachineObject *obj)
{
    ...
    // ============================================
    auto* pobj = as_phrozen(obj);
    std::string print_file = pobj ? pobj->GetPhrozenPrintFile() : std::string("");
...
void PhrozenStatusPanel::update_print_time(MachineObject *obj)
{
    ...
    // ============================================
    // print time
    float print_time = pobj ? pobj->GetPhrozenPrintTime() : 0.0f;
    // total process time = pre-process time + print time + end time
    float total_time = pobj ? pobj->GetPhrozenTotalTime() : 0.0f;
    ...
    if (print_status == "printing" || print_status == "paused") {
        std::string send_print_time = pobj ? pobj->GetPhrozenSendPrintTime() : std::string("");
...
void PhrozenStatusPanel::update_print_stage(MachineObject *obj)
{
    ...
    auto* pobj = as_phrozen(obj);
    std::string print_status = pobj ? pobj->GetPhrozenPrintStatus() : std::string("");
    float print_progress = pobj ? pobj->GetPhrozenPrintProgress() : 0.0f;
...
void PhrozenStatusPanel::update_thumbnail(MachineObject *obj)
{
    ...
    // 獲取當前打印的 GCode 文件名
    // ============================================
    std::string gcode_name = pobj ? pobj->GetPhrozenPrintFile() : std::string("");
...
void PhrozenStatusPanel::update_print_filament(MachineObject *obj)
{
    ...
    // ============================================
    auto* pobj = as_phrozen(obj);
    float print_filament = pobj ? pobj->GetPhrozenPrintFilamentAmount() : 0.0f;
        ...
        if (obj) {
            ...
            switch( eMoveType )
            {
                ...
                case PhrozenMovement::Nozzle_Home_XY:
                    BOOST_LOG_TRIVIAL(warning) << "Phrozen movement command not yet ported, type=" << (int)eMoveType;
                    break;
                default:
                    assert( 0 && "not implement" );
            }
