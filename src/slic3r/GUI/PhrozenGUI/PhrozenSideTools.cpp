...
//#include "../Widgets/bambu_networking.hpp"
...
namespace Slic3r { namespace GUI {
	PhrozenSideToolsPanel::PhrozenSideToolsPanel(wxWindow *parent, wxWindowID id, const wxPoint &pos, const wxSize &size)
{
    ...
    Bind(wxEVT_PAINT, &PhrozenSideToolsPanel::OnPaint, this);
    ...
    this->Bind(wxEVT_TIMER, &PhrozenSideToolsPanel::stop_interval, this);
    this->Bind(wxEVT_ENTER_WINDOW, &PhrozenSideToolsPanel::on_mouse_enter, this);
    this->Bind(wxEVT_LEAVE_WINDOW, &PhrozenSideToolsPanel::on_mouse_leave, this);
    this->Bind(wxEVT_LEFT_DOWN, &PhrozenSideToolsPanel::on_mouse_left_down, this);
    this->Bind(wxEVT_LEFT_UP, &PhrozenSideToolsPanel::on_mouse_left_up, this);
}

PhrozenSideToolsPanel::~PhrozenSideToolsPanel() { delete m_intetval_timer; }

void PhrozenSideToolsPanel::set_none_printer_mode()
{
    m_none_printer = true;
    Refresh();
}

void PhrozenSideToolsPanel::on_timer(wxTimerEvent &event)
{
}

void PhrozenSideToolsPanel::set_current_printer_name(std::string dev_name)
{
...
void PhrozenSideToolsPanel::set_current_printer_signal(WifiSignal sign)
{
...
void PhrozenSideToolsPanel::start_interval()
{
    m_intetval_timer->Start(PHROZEN_SIDE_TOOL_CLICK_INTERVAL);
    m_is_in_interval = true;
}

void PhrozenSideToolsPanel::stop_interval(wxTimerEvent& event)
{
...
bool PhrozenSideToolsPanel::is_in_interval()
{
    return m_is_in_interval;
}

void PhrozenSideToolsPanel::msw_rescale()
{
...
void PhrozenSideToolsPanel::OnPaint(wxPaintEvent &event)
{
    wxPaintDC dc(this);
    doRender(dc);
}

void PhrozenSideToolsPanel::render(wxDC &dc)
{
#ifdef __WXMSW__
    wxSize     size = GetSize();
    wxMemoryDC memdc;
...
void PhrozenSideToolsPanel::doRender(wxDC &dc)
{
    auto   left = FromDIP(15);
    wxSize size = GetSize();

    //if (m_none_printer) {
    //    dc.SetPen(PHROZEN_SIDE_TOOLS_LIGHT_GREEN);
    //    dc.SetBrush(PHROZEN_SIDE_TOOLS_LIGHT_GREEN);
    //    dc.DrawRectangle(0, 0, size.x, size.y);
    //}

    if (m_none_printer) {
        dc.SetPen(StateColor::darkModeColorFor(PHROZEN_SIDE_TOOLS_BRAND));   // ORCA: Sidebar header background color - Fix for dark mode compability
        dc.SetBrush(StateColor::darkModeColorFor(PHROZEN_SIDE_TOOLS_BRAND)); // ORCA: Sidebar header background color - Fix for dark mode compability
        dc.DrawRectangle(0, 0, size.x, size.y);
        ...
        dc.SetFont(::Label::Body_14);
        dc.SetBackgroundMode(wxTRANSPARENT);
        dc.SetTextForeground(StateColor::darkModeColorFor(PHROZEN_SIDE_TOOLS_GREY900));
    ...
    if (m_hover) {
        dc.SetPen(PHROZEN_SIDE_TOOLS_BRAND);
        dc.SetBrush(*wxTRANSPARENT_BRUSH);
        dc.DrawRectangle(0, 0, size.x, size.y);
    }
}

void PhrozenSideToolsPanel::on_mouse_left_down(wxMouseEvent &evt)
{
    m_click = true;
    Refresh();
}

void PhrozenSideToolsPanel::on_mouse_left_up(wxMouseEvent &evt)
{
     m_click = false;
     Refresh();
}

void PhrozenSideToolsPanel::on_mouse_enter(wxMouseEvent &evt)
{
    m_hover = true;
    Refresh();
}

void PhrozenSideToolsPanel::on_mouse_leave(wxMouseEvent &evt)
{
    m_hover = false;
    Refresh();
}

PhrozenSideTools::PhrozenSideTools(wxWindow *parent, wxWindowID id, const wxPoint &pos, const wxSize &size)
{
    wxPanel::Create(parent, id, pos, size);
    SetBackgroundColour(wxColour("#FEFFFF"));

    m_side_tools = new PhrozenSideToolsPanel(this, wxID_ANY);
...
void PhrozenSideTools::msw_rescale()
{
...
bool PhrozenSideTools::is_in_interval()
{
    return m_side_tools->is_in_interval();
}

void PhrozenSideTools::set_current_printer_name(std::string dev_name)
{
    m_side_tools->set_current_printer_name(dev_name);
}

void PhrozenSideTools::set_current_printer_signal(WifiSignal sign)
{
    m_side_tools->set_current_printer_signal(sign);
}

void PhrozenSideTools::set_none_printer_mode()
{
    m_side_tools->set_none_printer_mode();
}

void PhrozenSideTools::start_interval()
{
    m_side_tools->start_interval();
}

void PhrozenSideTools::update_connect_err_info(int code, wxString desc, wxString info)
{
...
void PhrozenSideTools::update_status(MachineObject* obj)
{
    if (!obj) return;
...
void PhrozenSideTools::show_status(int status)
{
    if (((status & (int)MonitorStatus::MONITOR_DISCONNECTED) != 0) || ((status & (int)MonitorStatus::MONITOR_DISCONNECTED_SERVER) != 0)) {
        ...
        m_connection_info->SetBackgroundColor(0xFF7C3F);
        m_connection_info->SetBorderColor(0xFF7C3F);
        m_connection_info->Show();
        m_more_button->Hide();
        m_side_error_panel->Hide();
    }
...
PhrozenSideTools::~PhrozenSideTools()
{
}
